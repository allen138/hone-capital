...
$(document).ready(function() {
  ...
  function renderRandom(res) {
    ...
    let dataBubbles = [];
    for (let i = 0; i < data.length; i++) {
      dataBubbles.push({ x: data[i].Volume_24h, y: data[i].Price, r: 10 });
    }
  ...
  $.ajax({
    ...
    const fiveThousandths = filterResponse.filter(
      res => res.Price_btc >= 0.005
    );
    ...
    const removeExistingBtc = removeEhereum.filter(
      res => res.Name !== "Monero"
    );
